...
export const DUNGEON_MOBS: Record<string, MobTemplate> = {
  ...
  // ---- Blightroot Hollow (5-player elite cave, ~L6-9) ----
  // A root-choked cavern under Bloomhaven Vale where the first rift's Corruption
  // pooled and took root. Bridges the starter zone into the Sunken Wastes.
  rootfen_crawler: {
    id: 'rootfen_crawler',
  ...
  rootwraith_the_hollowed: {
    ...
    moveSpeed: 7,
    aggroRadius: 16,
    aoePulse: { min: 11, max: 17, radius: 12, every: 10, name: 'Spore Burst' },
    loot: [
      { copper: 2200, chance: 1 },
      { itemId: 'cryptbone_greaves', chance: 0.34, rollGroup: 'rootwraith_guaranteed_uncommon' },
      { itemId: 'quilted_trousers', chance: 0.33, rollGroup: 'rootwraith_guaranteed_uncommon' },
      { itemId: 'oiled_boots', chance: 0.33, rollGroup: 'rootwraith_guaranteed_uncommon' },
      { itemId: 'greyjaw_hide_boots', chance: 0.22, rollGroup: 'rootwraith_bonus' },
      { itemId: 'cryptbone_helm', chance: 0.18, rollGroup: 'rootwraith_bonus' },
    ],
    scale: 1.4,
    color: 0x2f4f1a,
  },

  // ---- Emberhold Keep (5-player elite castle, ~L10-13) ----
  // A ruined desert garrison on the road up to Thornreach. When the second rift
  // opened its watch-fires never went out: the keep's knights burned and rose
  // again, Corruption-bound and wreathed in ember. A stone-hall crawl (reuses the
  // sanctum interior) that bridges the Sunken Wastes into the highlands.
  ashfell_knight: {
    id: 'ashfell_knight',
    name: 'Ashfell Knight',
    minLevel: 10,
    ...
    loot: [
      { copper: 140, chance: 1 },
      { itemId: 'bone_fragments', chance: 0.7 },
      { itemId: 'boundstone_girdle', chance: 0.06, rollGroup: 'ashfell_bonus' },
    ],
    scale: 1.1,
    color: 0xc25a2e,
    mortalStrike: { chance: 0.28, healReduction: 0.5, duration: 6, name: 'Searing Blow' },
  },
  emberbound_zealot: {
    id: 'emberbound_zealot',
    ...
    loot: [
      { copper: 160, chance: 1 },
      { itemId: 'linen_scrap', chance: 0.5 },
      { itemId: 'mistveil_grips', chance: 0.06, rollGroup: 'zealot_bonus' },
    ],
    desperateHeal: { belowHpPct: 0.3, healPct: 0.25 },
    scale: 1.0,
    color: 0xe08a2b,
  ...
  castellan_maldrek: {
    ...
    loot: [
      { copper: 480, chance: 1 },
      { itemId: 'boneguard_breastplate', chance: 0.4, rollGroup: 'maldrek_drop' },
      { itemId: 'tideguard_greaves', chance: 0.4, rollGroup: 'maldrek_drop' },
    ],
    scale: 1.2,
    color: 0xa8451f,
  },
  lord_kaelen_ashbound: {
    id: 'lord_kaelen_ashbound',
    ...
    moveSpeed: 7,
    aggroRadius: 16,
    aoePulse: { min: 14, max: 20, radius: 13, every: 9, name: 'Ember Nova' },
    loot: [
      { copper: 2600, chance: 1 },
      { itemId: 'boneplate_vest', chance: 0.34, rollGroup: 'kaelen_guaranteed' },
      { itemId: 'boundstone_helm', chance: 0.33, rollGroup: 'kaelen_guaranteed' },
      { itemId: 'tideguard_sabatons', chance: 0.33, rollGroup: 'kaelen_guaranteed' },
      { itemId: 'mistveil_cord', chance: 0.2, rollGroup: 'kaelen_bonus' },
      { itemId: 'eelscale_treads', chance: 0.18, rollGroup: 'kaelen_bonus' },
    ],
    scale: 1.45,
    color: 0x8b2f14,
  },
...
// Blightroot Hollow: crypt-paced cavern, packs of 2 to 3 corrupted beasts and
// weavers, the Hollowed Husk as miniboss, then Rootwraith on the root dais.
const BLIGHTROOT_SPAWN_LIST: DungeonSpawn[] = [
  { mobId: 'rootfen_crawler', x: -3, z: 18 },
  { mobId: 'spore_tick', x: 3, z: 19 },
  { mobId: 'rootfen_crawler', x: -9, z: 38 },
  { mobId: 'spore_tick', x: -5, z: 39 },
  { mobId: 'blightroot_weaver', x: 9, z: 54 },
  { mobId: 'rootfen_crawler', x: 5, z: 55 },
  { mobId: 'blightroot_weaver', x: -5, z: 68 },
  { mobId: 'spore_tick', x: -1, z: 70 },
  { mobId: 'hollowed_husk', x: -4, z: 82 },
  { mobId: 'rootfen_crawler', x: 1, z: 83 },
  { mobId: 'rootwraith_the_hollowed', x: 0, z: 98 },
  { mobId: 'blightroot_weaver', x: -4, z: 96 },
...
const EMBERHOLD_SPAWN_LIST: DungeonSpawn[] = [
  { mobId: 'ashfell_knight', x: -3, z: 18 },
  { mobId: 'ashfell_knight', x: 3, z: 19 },
  { mobId: 'ashfell_knight', x: -9, z: 38 },
  ...
  { mobId: 'lord_kaelen_ashbound', x: 0, z: 98 },
  { mobId: 'ashfell_knight', x: -4, z: 96 },
  { mobId: 'ashfell_knight', x: 4, z: 96 },
];
...
export const DUNGEON_DEFS: Record<string, DungeonDef> = {
  ...
  blightroot_hollow: {
    ...
    index: 6,
    doorPos: { x: -92, z: 66 }, // root-choked cave mouth west of the Sporewood
    entry: { x: 0, z: 4 },
    exitOffset: { x: 0, z: -6 },
    spawns: BLIGHTROOT_SPAWN_LIST,
    interior: 'crypt',
    suggestedPlayers: 5,
