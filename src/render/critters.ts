...
interface Critter {
  ...
  placed: boolean; // false until first scattered into the ring around the player
}
  ...
  return {
    ...
    update(px: number, pz: number, dt: number): void {
      ...
      for (let i = 0; i < critters.length; i++) {
        ...
        // Until first placement a critter still sits at its (0,0) init spot; never
        // show it there. Scatter it into the ring around the player first so the
        // pool doesn't pop in clustered at world origin and disperse outward.
        const dx = c.x - px,
          dz = c.z - pz;
        const dist = Math.hypot(dx, dz);
        if (!c.placed || dist > CULL_RADIUS || !validGround(c.x, c.z)) {
          relocate(c, px, pz);
          continue;
        }
