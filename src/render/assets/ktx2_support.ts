...
function detectViaThrowawayContext(loader: KTX2Loader): void {
  ...
  try {
    ...
    if (raw) {
      ...
      loader.workerConfig = {
        astcSupported: false,
        // Upstream also clears astcHDRSupported here. That flag postdates the
        // three version this repo pins (0.165 vs upstream's 0.185), and an
        // absent flag means the same thing as a false one, so it is omitted
        // rather than pulling a renderer-wide three upgrade into this change.
