...
function isPassiveAura(auraId: string): boolean {
  ...
  // Growverse divergence: upstream also checks `def.passive === true`. Growverse
  // abilities carry no `passive` flag, so the stance-group test is the whole rule.
  return def.exclusiveGroup?.endsWith('_stance') === true;
}
  ...
  handleSpellfx(ev: AbilityVfxSpellfxEvent): boolean {
    ...
    switch (ev.fx) {
      ...
      case 'selfCast': {
        ...
        // Growverse divergence: upstream also treats targetType 'any' as friendly
        // here. Growverse's targetType is only 'enemy' | 'friendly'.
        const friendly = targeted && defTargetType === 'friendly';
