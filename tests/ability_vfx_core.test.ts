...
describe('planCast / planImpact', () => {
  ...
  it('derives the marquee fields from the compact keys', () => {
    ...
    // Growverse divergence: upstream reads glacial_spike here, which this fork
    // has no ability for. Pyroblast caps projScale the same way (b.h 1.8, pw 1.5).
    const heavy = planCast(ABILITY_VFX_SPECS.pyroblast, 1);
    expect(heavy.projScale).toBe(PROJ_SCALE_MAX);
    // No Growverse ability spins today (upstream reads bladestorm), so the spin
    // key is covered against a literal spec rather than a content row.
    const storm2 = planCast({ c: '#ffffff', p: 'physical', spin: 1 }, 1);
