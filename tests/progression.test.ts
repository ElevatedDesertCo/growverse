...
describe('content referential integrity', () => {
  ...
  it('zones tile the world strip and content sits inside its zone band', () => {
    ...
    const problems: string[] = [];
    // Overworld content must tile the overworld zone strip. Realm content
    // (portal-reached dimensions) lives in a far coordinate band with its own
    // zone strip, validated by tests/realms.test.ts, so it is exempt here.
