...
import { BUD_GRADES } from '../src/sim/genetics';
...
// Cultivation is a fourth acquisition source alongside mob loot, ground objects, and
// scripted grants: anything a garden plot can yield. Derived from the content rather
// than allowlisted, so a typo'd bud id in a quest objective still fails this guard.
const CULTIVATED_ITEMS = new Set<string>([
  ...Object.values(PLANTS).flatMap((p) => p.yields.map((y) => y.itemId)),
  ...BUD_GRADES,
]);
  ...
  it('every collect objective is obtainable', () => {
    ...
    for (const q of Object.values(QUESTS)) {
      for (const obj of q.objectives) {
        ...
        const fromCultivation = CULTIVATED_ITEMS.has(obj.itemId);
        if (!fromLoot && !fromGround && !fromScript && !fromCultivation) {
