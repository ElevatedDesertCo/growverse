...
describe('validateGuildName', () => {
  ...
  // Guild names are player-authored text shown in chat headers and rosters, so
  // they carry the same content gate as account and character names. This used
  // to be shape-only, which let an offensive guild name through.
  it('rejects offensive names, including confusable spellings', () => {
    expect(validateGuildName('Shit Lords')).toBeNull();
    expect(validateGuildName('The Fuckers')).toBeNull();
    // the shared matcher folds digit/symbol confusables, so leetspeak is caught
    expect(validateGuildName('Sh1t Lords')).toBeNull();
  });
  it('still accepts ordinary names that merely contain a flagged substring', () => {
    // the matcher is word-aware, so an innocent name embedding a short term stands
    expect(validateGuildName('Grasshoppers')).toBe('Grasshoppers');
    expect(validateGuildName('Scunthorpe Guard')).toBe('Scunthorpe Guard');
  });
});
