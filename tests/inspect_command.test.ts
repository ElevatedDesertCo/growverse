...
describe('/inspect command', () => {
  it("reports another player's level, class, and health", () => {
    ...
    expect(inspectReply(sim, a, '/inspect Bet')).toBe('Bet: Level 8 Embercaster, HP 100%.');
  });
  ...
  it('shows a partial-health percentage and "dead" for a corpse', () => {
    ...
    expect(inspectReply(sim, a, '/inspect Gimel')).toBe(
      `Gimel: Level ${e.level} Sandrunner, HP 40%.`,
    );

    e.hp = 0;
    expect(inspectReply(sim, a, '/inspect Gimel')).toBe(
      `Gimel: Level ${e.level} Sandrunner, HP dead.`,
    );
  });
  ...
  it('matches names case-insensitively when unambiguous', () => {
    ...
    expect(inspectReply(sim, a, '/inspect bet')).toMatch(/^Bet: Level \d+ Embercaster/);
  });
  ...
  it('supports the /ins and /examine aliases', () => {
    ...
    const a = sim.addPlayer('warrior', 'Aleph');
    sim.addPlayer('mage', 'Bet');
    expect(inspectReply(sim, a, '/ins Bet')).toMatch(/^Bet: Level \d+ Embercaster/);
    expect(inspectReply(sim, a, '/examine Bet')).toMatch(/^Bet: Level \d+ Embercaster/);
  });
});
