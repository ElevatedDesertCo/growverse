...
describe('delve spatial band', () => {
  ...
  it('isDelvePos covers the full room footprint west of DELVE_X_MIN (regression: camera yank bug)', () => {
    // Rooms are ~50 u wide, centred at DELVE_X_MIN (6000). Delve side walls sit at
    // instance-local |x| = 25 (delve_layout WALL_X), collider outer face at |x| = 26,
    // i.e. world-x = 5974 (slot 0). Before DELVE_BAND_X_MIN, the west half of the room
    ...
    expect(isDelvePos(origin.x - 2)).toBe(true); // 5998, exact repro coordinate
    expect(isDelvePos(origin.x - 22)).toBe(true); // walkable west edge
    expect(isDelvePos(origin.x - 26)).toBe(true); // west wall outer face (5974)
    ...
    expect(delveAt(origin.x - 26)?.index).toBe(0); // west wall outer face (5974)
    expect(delveAt(origin.x)?.index).toBe(0); // room centre

    // Arena still classifies correctly (arena instances live at ARENA_X = 5400)
    expect(isArenaPos(ARENA_X)).toBe(true);
    expect(isDelvePos(ARENA_X)).toBe(false);
  });
  ...
  it('pins the absolute 6000 boundary against the arena seam (relocation regression)', () => {
    ...
    // then 4800 -> 6000 (arena to x=5400) when Blightroot Hollow claimed dungeon
    // index 6 and the arena/delve bands shifted past it. Pin the load-bearing
    // constant and the exact arena/delve seam so a future arena or delve respacing
    // that re-introduces overlap fails here.
...
describe('delve reward chest + surface exit flow', () => {
  ...
  it('the Bountiful roll is deterministic for a given seed', () => {
    ...
    // yields the same outcome; seed 14 is a known Bountiful roll under the current
    // world layout (the fixtures below don't rely on it, they force run.bountiful).
