...
describe('resolveTokenMint env precedence', () => {
  it('prefers the $GROW names over the legacy WOC names, server var first per family', () => {
    expect(
      resolveTokenMint({
        GROW_MINT: 'grow',
        VITE_GROW_MINT: 'vite-grow',
        WOC_MINT: 'woc',
        VITE_WOC_MINT: 'vite-woc',
      }),
    ).toBe('grow');
    expect(resolveTokenMint({ VITE_GROW_MINT: 'vite-grow', WOC_MINT: 'woc' })).toBe('vite-grow');
    expect(resolveTokenMint({ WOC_MINT: 'woc', VITE_WOC_MINT: 'vite-woc' })).toBe('woc');
    expect(resolveTokenMint({ VITE_WOC_MINT: 'vite-woc' })).toBe('vite-woc');
  });

  it('skips set-but-empty and whitespace-only vars instead of breaking balance reads', () => {
    expect(resolveTokenMint({ GROW_MINT: '', WOC_MINT: 'woc' })).toBe('woc');
    expect(resolveTokenMint({ GROW_MINT: '   ', VITE_WOC_MINT: 'vite-woc' })).toBe('vite-woc');
    expect(resolveTokenMint({ GROW_MINT: '  grow  ' })).toBe('grow');
  });
