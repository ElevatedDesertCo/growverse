...
// A zone that carries both POIs and an in-band overworld dungeon entrance, so the
// pois/portals projection is actually exercised. Not assumed to be ZONES[0]: the
...
function makeDelveWorld(shape: 'sim' | 'client'): IWorld {
  ...
  return {
    player: { id: 1, kind: 'player', name: 'Me', pos: { x: 6000, z: 0 }, facing: 0, ...simJunk },
    entities: new Map(),
    socialInfo: null,
    delveRun: { delveId: 'd', modules: ['m'], moduleIndex: 0, origin: { x: 6000, z: 0 } },
    cfg: { seed: 42, playerClass: 'warrior' },
    playerId: 1,
