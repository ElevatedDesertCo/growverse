...
describe('creator: the customizer mount point exists on every entry that creates', () => {
  ...
  it('play.html carries both hosts (it ships the offline panel too)', () => {
    // Diverges from upstream on purpose: WOCC's play.html has no
    // #offline-select panel, so there the offline host is index-only. This
    // entry DOES carry that panel, so it creates offline characters as well
    // and needs both mount points or the offline flow loses the customizer
    // and the skin row it replaced, leaving a bare "Appearance" header.
    const html = read('play.html');
    expect(html).toContain('id="offline-select"');
    for (const id of hostIds(read('src/main.ts'))) {
      expect(html, id).toContain(`id="${id}"`);
    }
  });
