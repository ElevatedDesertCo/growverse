...
export function getEmailService(): EmailService {
  if (!singleton) {
    ...
    singleton = new EmailService({
      ...
      log: (entry) =>
        void recordEmailLog(entry).catch((err) => console.error('email_log write failed:', err)),
    });
...
// Absolute base for links inside emails (verify, unsubscribe). Prefers an explicit
// EMAIL_BASE_URL / PUBLIC_BASE_URL / PUBLIC_ORIGIN; with none set it falls back to
// '' (relative), which no mail client can open, so production MUST set one of these
// env vars. There is deliberately no baked-in domain default.
export function emailBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return (env.EMAIL_BASE_URL || env.PUBLIC_BASE_URL || env.PUBLIC_ORIGIN || '').replace(/\/+$/, '');
}
