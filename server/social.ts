...
//
// The one import is `offensiveName` from ./auth, and it is safe for that
// decoupling: auth's module load only builds a regex matcher, and its banlist
// file read is lazy, env-gated, and try/caught, so an in-memory fake test still
// runs without Postgres, sockets, or a banlist on disk.
...
export function validateGuildName(name: string): string | null {
  ...
  // A guild name is player-authored text rendered in chat channel headers and
  // rosters, so it gets the SAME content gate as account and character names
  // (profanity matcher + operator banlist + confusable folding). It previously
  // had shape rules only. The caller's rejection message stays generic on
  // purpose: naming the reason would let someone binary-search the word list.
  if (offensiveName(trimmed)) return null;
  return trimmed;
}
