// Pure (IO-free) logic for the Growverse Discord bot: Gateway intents,
...
export const SLASH_COMMANDS = [
  { name: 'whoami', description: 'Show your Growverse link status and reward points' },
  { name: 'link', description: 'Get the link to connect your Discord to Growverse' },
] as const;
...
export function buildWhoamiContent(roles: {
  ...
  if (!roles.linked) {
    return 'Your Discord is not linked to a Growverse account yet. Use /link to connect it and start earning rewards.';
  }
...
export function buildLinkContent(gameUrl: string): string {
  return `Connect your Discord to Growverse to earn rewards and flex your characters: open ${gameUrl}, log in, and press the Discord button in the game HUD (or "Continue with Discord" on the login screen).`;
}

/** Welcome message for a new guild member. */
export function buildWelcomeMessage(opts: { userMention: string; gameUrl: string }): string {
  return `Welcome to Growverse, ${opts.userMention}! Play at ${opts.gameUrl} and link your Discord in the game HUD to earn rewards, claim swag, and rank up here in the server.`;
}
