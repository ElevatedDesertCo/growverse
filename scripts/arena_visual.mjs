...
const panelOpen = await pageA.evaluate(
  () => document.querySelector('#arena-window')?.style.display === 'block',
);
...
await pageA.waitForFunction(() => window.__game.world.arenaInfo?.match != null, {
  timeout: 15000,
  polling: 200,
});
...
await pageA.evaluate(() => {
  window.__game.input.camDist = 19;
  window.__game.input.camPitch = 0.62;
});
await sleep(3000);
await pageA.bringToFront();
...
await pageA.waitForFunction(() => window.__game.world.arenaInfo?.match?.state === 'active', {
  timeout: 12000,
  polling: 200,
});
console.log('bout is live');
const bPos = await pageB.evaluate(() => ({
...
await pageA.evaluate(
  (p) => window.__game.online.cmd({ cmd: 'dev_teleport', x: p.x + 3, z: p.z }),
  bPos,
);
await sleep(500);
const swing = async () => {
