...
await page.evaluate((s) => {
  ...
  ctx.font = '15px system-ui, sans-serif';
  ctx.fillText(
    `final: form ${fd(s.form)} yd, none ${fd(s.none)} yd  (ratio ${(s.form.at(-1) / s.none.at(-1)).toFixed(2)}x)`,
    PAD,
    H - 18,
  );
}, series);
...
await page.evaluate(() => {
  const cv = document.querySelector('#tf-chart');
  if (cv) cv.remove();
...
console.log(
  ...
  'form',
  series.form.at(-1).toFixed(1),
  'ratio form/none',
  (series.form.at(-1) / series.none.at(-1)).toFixed(2),
);
