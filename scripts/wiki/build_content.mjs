...
const entrySource = `
  ...
  export { MOUNT_DEFS, SWIFT_SPEED_BONUS } from './src/sim/content/mounts.ts';
...
// Mounts: the stable roster, spoiler-safe. Only structural facts surface (display name,
// the level gate, riding vs swift tier, the $GROW-exclusive flag) in the registry's
// declaration order. NEVER the speed bonuses or the copper/$GROW prices: those are
// balance/economy values, not public reference. The tier is a boolean derived from the
// speed constants so no number is baked.
const mounts = Object.values(MOUNT_DEFS).map((m) => ({
  id: m.id,
  name: m.name,
  requiredLevel: m.requiredLevel,
  swift: m.speedBonus >= SWIFT_SPEED_BONUS,
...
writeFileSync(
  ...
  [
    ...
    `\nexport const GUIDE_MOUNTS: GuideMount[] = ${JSON.stringify(mounts, null, 2)};\n`,
    `\nexport const GUIDE_MODELS: Record<string, GuideModelSpec> = ${JSON.stringify(MODELS, null, 2)};\n`,
  ].join(''),
);
// eslint-disable-next-line no-console
console.log(
  `generated src/guide/content.generated.ts (${classes.length} classes, ${zones.length} zones, ${dungeons.length} dungeons, ${warlockPets.length} warlock pets, ${families.length} families, ${delves.length} delves, ${mounts.length} mounts, ${Object.keys(MODELS).length} models)`,
);
