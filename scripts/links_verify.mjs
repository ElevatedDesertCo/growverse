...
async function main() {
  ...
  try {
    ...
    ok(title === 'Growverse - Official Links', `document.title is localized ("${title}")`);
    ...
    ok(
      EXPECTED_LINKS.filter((u) => u !== 'https://growverse.vercel.app/').every((u) =>
        sameAs.includes(u),
      ),
