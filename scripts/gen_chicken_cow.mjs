...
const matHorn = M('horn', [0.8, 0.76, 0.68]);
...
const matGold = doc
  .createMaterial('gold')
  .setBaseColorFactor([0.97, 0.8, 0.27, 1])
...
function quatY(d) {
  ...
  const ax = [d[2], 0, -d[0]];
  const L = Math.hypot(ax[0], ax[1], ax[2]); // cross(Y,d)
...
// ---- assemble ---------------------------------------------------------------
const root = group('ChickenCow');
scene.addChild(root);
const pose = group('pose');
root.addChild(pose); // death/jump
const body = group('body');
pose.addChild(body); // idle bob + waddle
...
const SPOTS = [
  [0.37, 1.1, 0.22, 0.13, 0.15],
  [0.32, 0.86, -0.3, 0.12, 0.13],
  [-0.36, 1.02, 0.06, 0.14, 0.16],
  [-0.3, 1.18, -0.24, 0.1, 0.12],
  [0.1, 1.36, -0.02, 0.13, 0.1],
  [0.06, 0.74, 0.4, 0.12, 0.13],
  [-0.12, 0.8, -0.46, 0.11, 0.12],
  [0.34, 1.06, -0.42, 0.1, 0.11],
  [-0.4, 0.92, -0.4, 0.09, 0.1],
];
for (const [x, y, z, rxz, ry] of SPOTS)
  body.addChild(part('spot', ellipsoid(rxz, ry, rxz, 10, 7), matSpot, [x, y, z]));

// udder + teats (rear underside)
body.addChild(part('udder', ellipsoid(0.18, 0.14, 0.2), matUdder, [0, 0.64, -0.2]));
for (const dx of [-0.08, 0.08])
  for (const dz of [-0.07, 0.07])
    body.addChild(part('teat', frustum(0.035, 0.02, 0.09), matUdder, [dx, 0.5, -0.2 + dz]));

// wings (flap about z)
const wingL = group('wingL', [0.39, 1.08, 0.0]);
body.addChild(wingL);
wingL.addChild(part('wingLm', ellipsoid(0.07, 0.22, 0.38), matBody, [0.04, -0.13, 0]));
const wingR = group('wingR', [-0.39, 1.08, 0.0]);
body.addChild(wingR);
wingR.addChild(part('wingRm', ellipsoid(0.07, 0.22, 0.38), matBody, [-0.04, -0.13, 0]));

// fanned tail feathers
const tail = group('tail', [0, 1.1, -0.64], qx(0.8));
body.addChild(tail);
...
body.addChild(part('neck', frustum(0.18, 0.12, 0.32), matBody, [0, 1.0, 0.4], qx(0.95)));
const head = group('head', [0, 1.26, 0.6]);
body.addChild(head);
...
for (const dz of [-0.1, 0, 0.1])
  head.addChild(part('comb', ellipsoid(0.05, 0.11, 0.08), matComb, [0, 0.24, dz]));
...
const bell = group('bell', [0, 0.92, 0.7]);
body.addChild(bell);
bell.addChild(bone('strapL', [0, 0.07, 0], [0.24, 0.26, -0.3], 0.024, 0.018, matStrap));
bell.addChild(bone('strapR', [0, 0.07, 0], [-0.24, 0.26, -0.3], 0.024, 0.018, matStrap));
...
function leg(name, side) {
  const hip = group(name, [side * 0.15, 0.66, 0.02]);
  pose.addChild(hip);
  const knee = [0, -0.28, 0.12],
    ankle = [0, -0.5, -0.06],
    footEnd = [0, -0.66, 0.06];
  ...
  for (const [tx, tz] of [
    ...
    hip.addChild(
      part(
        `${name}_toe`,
        frustum(0.02, 0.0, 0.17),
        matHoof,
        [footEnd[0] + tx, footEnd[1] + 0.005, footEnd[2] + tz],
        qx(Math.PI / 2.1),
      ),
    );
  hip.addChild(
    part(
      `${name}_spur`,
      frustum(0.016, 0.0, 0.09),
      matHoof,
      [footEnd[0], footEnd[1] + 0.01, footEnd[2] - 0.09],
      qx(-Math.PI / 2.1),
    ),
  );
  return hip;
}
const legL = leg('legL', 1),
  legR = leg('legR', -1);
