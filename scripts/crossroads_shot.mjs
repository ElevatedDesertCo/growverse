...
await page.evaluate(
  (pack, equip) => {
    ...
    const merchant = ents.find(
      (e) => e && (e.templateId === 'the_merchant' || e.npcId === 'the_merchant'),
    );
    if (merchant && merchant.pos) {
...
await page.evaluate(() => {
  ...
  for (let i = 0; i < 3 && getComputedStyle(el).display === 'none'; i++)
