...
const result = await page.evaluate(() => {
  ...
  const first = [...sim.entities.values()].find(
    (e) => e.templateId === 'bound_guardian' && !e.dead,
  );
  sim.entities.delete(first.id);
  const goneAfterDespawn = ![...sim.entities.values()].some(
    (e) => e.templateId === 'bound_guardian' && !e.dead,
  );
  ...
  guardian.pos.x = p.pos.x + 6;
  guardian.pos.z = p.pos.z;
...
console.log(
  'saved tmp/bound_guardian_resummon_full.png, tmp/bound_guardian_resummon_targetframe.png',
);
await browser.close();
