...
function newSim() {
  ...
  set(pet, PET.x, PET.z);
  set(owner, OWN.x, OWN.z);
  return { sim, owner, pet };
}
function traceAfter() {
  const { sim, owner, pet } = newSim();
  const path = [{ ...pet.pos }];
  for (let i = 0; i < 20 * 10 && dist2d(pet.pos, owner.pos) > 3.5; i++) {
    sim.tick();
    path.push({ ...pet.pos });
  }
  return path;
...
function traceBefore() {
  ...
  for (let i = 0; i < 20 * 10; i++) {
    ...
    if (d <= 3.5) break;
    if (d > 60) pet.pos = { ...owner.pos };
    else sim.moveToward(pet, owner.pos, speed);
    path.push({ ...pet.pos });
  }
  return path;
}
const before = traceBefore(),
  after = traceAfter();
...
const VX0 = -34,
  ...
  S = 24,
  PAD = 26;
const W = (VX1 - VX0) * S + PAD * 2,
  H = (VZ1 - VZ0) * S + PAD * 2;
const sx = (x) => PAD + (x - VX0) * S,
  sy = (z) => H - PAD - (z - VZ0) * S;
let cells = '';
for (let x = VX0; x < VX1; x += 0.5)
  for (let z = VZ0; z < VZ1; z += 0.5)
    if (isBlocked(SEED, x, z, 0.5))
      cells += `<rect x="${sx(x)}" y="${sy(z + 0.5)}" width="${0.5 * S}" height="${0.5 * S}" fill="#5a4632"/>`;
const poly = (p, c) =>
  `<polyline fill="none" stroke="${c}" stroke-width="3" stroke-linejoin="round" points="${p.map((q) => `${sx(q.x).toFixed(1)},${sy(q.z).toFixed(1)}`).join(' ')}"/>`;
...
process.stderr.write(
  `before ends ${dist2d(before.at(-1), OWN).toFixed(1)}yd away; after ends ${dist2d(after.at(-1), OWN).toFixed(1)}yd away\n`,
);
