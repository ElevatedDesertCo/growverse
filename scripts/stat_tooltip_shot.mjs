...
page.on('console', (m) => {
  if (m.type() === 'error' && !/502|project stats/.test(m.text()))
...
const setTouch = (on) =>
  page.evaluate((v) => document.body.classList.toggle('mobile-touch', v), on);
...
const ttMetrics = () =>
  page.evaluate(() => {
    ...
    return {
      ...
      overflow:
        r.right > window.innerWidth || r.left < 0 || r.top < 0 || r.bottom > window.innerHeight,
    };
  });

// Every .stat-cell box: min/max width+height and how many fall under the touch floor.
const cellMetrics = (floor) =>
  page.evaluate((f) => {
...
async function hoverShot(stat, file) {
  await page.evaluate((s) => {
    document
      .querySelector(`.char-stats [data-stat="${s}"]`)
...
async function reopenChar() {
  if (!(await page.evaluate(() => !!(window.__game && window.__game.sim))))
    throw new Error('lost game on resize');
  await page.evaluate(() => {
    const h = window.__game.hud;
    if (document.querySelector('#char-window').style.display === 'block') h.toggleChar();
    h.hideTooltip();
...
results.mobile_portrait_armor = await longPressShot(
  'armor',
  'stat_tooltip_mobile_portrait_armor.png',
);
...
results.mobile_landscape_armor = await longPressShot(
  'armor',
  'stat_tooltip_mobile_landscape_armor.png',
);
