...
| Mobile / a11y regression | `mobile_input_zoom_check.mjs` (the 16px input-font floor across the shell), `mobile_touch_target_check.mjs` (per-window: the 40x40 rendered tap-target floor, no horizontal page overflow, and the Tab-trap / return-focus contract, in phone portrait AND landscape), `vendor_focus_check.mjs` (the CO-ROOT trap: the Tab cycle must span the vendor window and the #bags grid it co-opens, and never leave the pair), `growverse_district_check.mjs` (NPC terrain seating, building footprints, station separation, plus district + window screenshots) | dev |
