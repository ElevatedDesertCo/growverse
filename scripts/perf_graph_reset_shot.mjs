...
async function shoot(name) {
  ...
  console.log(
    `${name}:`,
    JSON.stringify({ panelW: box.panelW, graphW: box.graphW, graphCssW: box.graphCssW }),
  );
  return box;
}
...
await page.waitForFunction(() => document.querySelectorAll('#perf-overlay .perf-row').length <= 1, {
  timeout: 8000,
});
...
console.log(
  `graph shrank on Minimal: ${shrank} (everything=${everything.graphW}px -> minimal=${minimal.graphW}px)`,
);
