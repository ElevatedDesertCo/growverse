...
if [ "${seed_hash}" != "${current_hash}" ]; then
  echo "Importing Growverse wiki seed pages..."
  printf '%s\n' 'Main Page' | php maintenance/deleteBatch.php --u "${MEDIAWIKI_ADMIN_USER}" --r 'Replace stock install page with Growverse seed' || true
